...
if (favicon) {
  ...
  function renderContact() {
    ...
    // Copy-Discord row
    const discordRow = document.getElementById("contact-discord-row");
    if (c.discordUsername) {
      document.getElementById("contact-discord-text").innerHTML = `${ICONS.discord}<span>${esc(c.discordUsername)}</span>`;
      document.getElementById("contact-discord-text").style.display = "flex";
      document.getElementById("contact-discord-text").style.alignItems = "center";
      document.getElementById("contact-discord-text").style.gap = "8px";
      const discordBtn = document.getElementById("copy-discord-btn");
      discordBtn.addEventListener("click", async () => {
        try {
          await navigator.clipboard.writeText(c.discordUsername);
          discordBtn.textContent = "Copied!";
          discordBtn.classList.add("copied");
          setTimeout(() => {
            discordBtn.textContent = "Copy";
            discordBtn.classList.remove("copied");
          }, 1800);
        } catch (e) {
          discordBtn.textContent = "Press Ctrl+C";
        }
      });
    } else {
...
function getEmbedUrl(type, src) {
 ...
 function wireThemeToggle() {
  ...
  /* ---------------------------------------------------------------------
     SECTION NAVIGATOR — fixed bottom-right arrow that jumps to the next
     section and labels which section is coming up next.
     ------------------------------------------------------------------- */
  function wireSectionNav() {
    const nav = document.getElementById("section-nav");
    const btn = document.getElementById("section-nav-btn");
    const label = document.getElementById("section-nav-label");
    if (!nav || !btn || !label) return;

    // Map section id -> readable name, taken straight from the main nav links
    const nameMap = new Map(
      Array.from(document.querySelectorAll(".main-nav a")).map((a) => [
    ...
    function update() {
      ...
      if (next) {
        label.textContent = nameMap.get(next.id) || next.id;
        nav.classList.remove("is-last");
        btn.setAttribute("aria-label", "Scroll to " + label.textContent);
      } else {
        label.textContent = "Back to top";
        nav.classList.add("is-last");
